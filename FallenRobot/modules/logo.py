...
async def lego(event):
    ...
    if event.sender_id != OWNER_ID and not quew:
        ...
        return
    pesan = await event.reply("**ᴄʀᴇᴀᴛɪɴɢ ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛᴇᴅ ʟᴏɢᴏ from 「 sᴜᴋᴜɴ ☯︎ ʀᴏʙᴏᴛ 」.ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ ᴀ sᴇᴄ​...**")
    try:
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ 「 sᴜᴋᴜɴ ☯︎ ʀᴏʙᴏᴛ 」"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @sukunsupports")


__mod_name__ = "Lᴏɢᴏ​"

__help__ = """
「 sᴜᴋᴜɴ ☯︎ ʀᴏʙᴏᴛ 」 can create some beautiful and attractive logo for your profile pics.
